...
const SLIDES = [
  {
    ...
    image: 'https://images.unsplash.com/photo-1618512496248-a07fe83aa8cf?q=80&w=1000&auto=format&fit=crop',
...
export const HeroCarousel: React.FC = () => {
  ...
  return (
    <section className="relative overflow-hidden bg-brand-deep text-white rounded-3xl my-4 mx-4 sm:mx-6 lg:mx-8 shadow-2xl border border-brand-800">
      <div className={`relative min-h-[420px] md:min-h-[480px] flex items-center bg-gradient-to-r ${slide.bgGradient} transition-all duration-700`}>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10 py-12 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10 w-full">
          
          {/* Text Content Column */}
          <div className="lg:col-span-7 space-y-4">
            
            <div className="flex items-center gap-2">
              <span className="bg-gold-500/20 text-gold-300 border border-gold-400/40 text-xs font-black px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-gold-400" />
                {slide.tag}
              </span>
              <span className="bg-emerald-500/30 text-emerald-200 text-xs font-bold px-2.5 py-1 rounded-full flex items-center gap-1">
                <Plane className="w-3.5 h-3.5 text-amber-300" /> {slide.badge}
              </span>
            </div>

            <h1 className="font-serif font-extrabold text-3xl sm:text-4xl lg:text-5xl leading-tight tracking-tight text-white drop-shadow-md">
              {slide.title}
            </h1>
            ...
            <div className="pt-2 flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-3 text-xs text-emerald-200 font-semibold border-l border-brand-700/80 pl-4">
